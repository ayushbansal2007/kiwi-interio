...
import { Activity, ArrowUpRight, BarChart3, Bot, Clock3, Mail, MessageCircleMore, Phone, RefreshCw, TicketCheck, UserRound, Users, X } from "lucide-react";
...
export default function AdminAnalytics() {
  ...
  const openConversation = async (user: User) => {
    ...
    try {
      const response = await apiClient(`${API_BASE_URL}/api/admin/users/${user._id}/conversations?limit=100`);
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.message || "Could not load conversation");
  ...
  return <div className="space-y-7 pb-10">
    <section className="relative overflow-hidden rounded-[28px] bg-neutral-950 px-6 py-7 text-white sm:px-8 sm:py-9">
      ...
      <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-neutral-300">Track customer engagement, AI demand and support activity from one calm, focused workspace.</p>
        </div>
    ...
    <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {metrics.map(({ label, value, icon: Icon, tint, note }) => <article key={label} className="group rounded-3xl border border-neutral-200/80 bg-white p-5 shadow-[0_10px_30px_-22px_rgba(0,0,0,0.45)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_38px_-22px_rgba(0,0,0,0.35)]">
    ...
    <section className="overflow-hidden rounded-3xl border border-neutral-200/80 bg-white">
      <div className="flex flex-col justify-between gap-3 border-b border-neutral-100 px-5 py-5 sm:flex-row sm:items-center sm:px-6"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-red-600">Customer directory</p><h3 className="mt-1 text-xl font-black tracking-[-0.03em] text-neutral-950">Open a customer conversation</h3></div><span className="rounded-full bg-neutral-100 px-3 py-1.5 text-xs font-bold text-neutral-600">{users.length} customers</span></div>
      <div className="max-h-[480px] divide-y divide-neutral-100 overflow-auto">{users.length ? users.map((user) => <button key={user._id} onClick={() => void openConversation(user)} className="group flex w-full flex-col gap-4 px-5 py-4 text-left transition hover:bg-red-50/45 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div className="flex min-w-0 items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-neutral-100 text-sm font-black text-neutral-600">{user.name.charAt(0).toUpperCase()}</span><span className="min-w-0"><span className="block truncate text-sm font-bold text-neutral-900">{user.name}</span><span className="mt-0.5 flex flex-wrap gap-x-3 gap-y-1 text-xs text-neutral-500"><span className="inline-flex items-center gap-1"><Mail size={12} /> {user.email}</span>{user.number && <span className="inline-flex items-center gap-1"><Phone size={12} /> {user.number}</span>}</span></span></div><div className="flex shrink-0 items-center gap-4 pl-[52px] sm:pl-0"><span className="text-xs text-neutral-400"><Clock3 size={12} className="mr-1 inline" /> {formatDate(user.lastLoginAt)}</span><span className="rounded-full bg-neutral-950 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-white transition group-hover:bg-red-600">{user.chatCount} chats</span></div></button>) : <p className="p-7 text-sm text-neutral-400">No customer accounts found.</p>}</div>
    </section>

    {selectedUser && <div className="fixed inset-0 z-[60] flex justify-end bg-neutral-950/35 p-0 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Customer conversation">
      <section className="flex h-full w-full max-w-2xl flex-col bg-[#fffcf8] shadow-2xl">
        <header className="flex items-start justify-between gap-4 border-b border-neutral-200 bg-white px-5 py-5 sm:px-7"><div className="flex gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-red-50 text-red-600"><UserRound size={20} /></span><div><h3 className="text-lg font-black tracking-[-0.03em] text-neutral-950">{selectedUser.name}</h3><p className="text-xs text-neutral-500">{selectedUser.email}{selectedUser.number ? ` · ${selectedUser.number}` : ""}</p></div></div><button onClick={() => setSelectedUser(null)} className="grid h-9 w-9 place-items-center rounded-full border border-neutral-200 text-neutral-600 transition hover:bg-neutral-950 hover:text-white" aria-label="Close conversation"><X size={18} /></button></header>
        <div className="flex-1 space-y-4 overflow-auto px-5 py-6 sm:px-7">{conversationLoading ? <div className="grid h-40 place-items-center"><span className="h-7 w-7 animate-spin rounded-full border-2 border-red-100 border-t-red-600" /></div> : conversation.length ? conversation.map((chat) => <article key={chat._id} className={`max-w-[92%] rounded-3xl px-4 py-3.5 ${chat.role === "user" ? "mr-auto rounded-tl-md bg-white shadow-sm ring-1 ring-neutral-200" : "ml-auto rounded-tr-md bg-neutral-950 text-white"}`}><div className={`flex items-center justify-between gap-4 text-[10px] font-bold uppercase tracking-wider ${chat.role === "user" ? "text-neutral-400" : "text-neutral-400"}`}><span>{chat.role === "user" ? "Customer" : "Kiwi AI"}{chat.data?.category ? ` · ${chat.data.category}` : ""}</span><time>{formatDate(chat.createdAt)}</time></div><p className={`mt-2 whitespace-pre-wrap text-sm leading-6 ${chat.role === "user" ? "text-neutral-700" : "text-neutral-100"}`}>{chat.message}</p></article>) : <div className="grid h-48 place-items-center rounded-3xl border border-dashed border-neutral-200 bg-white text-center"><p className="text-sm text-neutral-400">No AI messages stored for this customer yet.</p></div>}</div>
      </section>
    </div>}
  </div>;
}
